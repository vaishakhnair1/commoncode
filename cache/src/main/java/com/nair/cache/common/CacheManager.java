...
/**
 * @author vaishakh
 * This is the Manager class for the Caching Solution.
 * It abstracts the internal functionalities from the consumer of the APIs.
 * It eagerly loads an instance of itself, which contains a Registry for all caches registered,
 * and secondary data sources for those which have been supplied.
 */
public final class CacheManager {
	...
	/**
	 * The Registry for the Secondary data sources for all Expirable Caches that have been procided.
	 */
	 ...
	 * @return the single instance of {@code CacheManager} that is shared across the JVM}
	 */
	 ...
	 * @param expirableCacheName the name with which the expirable cache is to be registered
	 * @param keyClass the class which acts as the key
	 * @param valueClass the value which acts as the value
	 * @throws InstanceAlreadyExistsException if an expirable cache with the same name has already been registered.
	 */
	 ...
	 * @param keyClass the class which acts as the key
	 * @param valueClass the value which acts as the value
	 * @param timeOut the time out for the keys
	 * @param timeUnit the Units in which above time out is calculated
	 * @throws InstanceAlreadyExistsException if an expirable cache with the same name has already been registered.
	 */
	public <K, V> void registerExpirableCache(final String expirableCacheName, final Class<K> keyClass, final Class<V> valueClass,
	 ...
	 * @param keyClass the class which acts as the key
	 * @param valueClass the value which acts as the value
	 * @param secondaryDataSource The secondary data source from which data is to be fetched if value is not found in Cache.
	 * If value is found in secondary data source, then same will be populated to Cache, with timeout.
	 * @throws InstanceAlreadyExistsException if an expirable cache with the same name has already been registered.
	 */
	public <K, V> void registerExpirableCache(final String expirableCacheName, final Class<K> keyClass, final Class<V> valueClass,
	 ...
	 * @param expirableCacheName the name with which the expirable cache is to be registered
	 * @param expirableCache the instance of the Expirable Cache
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Used to fetch a Value from the Expirable Cache.
	 * If Value is not found in Cache, and Secondary Data Source is configured for the cache, value is fetched from the same and populated in Cache.
	 * @param k the Key.
	 * @param clazz The class in which response is expected
	 * @param registeredExpirableCache The name with which the cache has been registered.
	 * @return The value that is present in the cache, or one that was loaded from Secondary Data Source(If configured)
	 */
	...
	/**
	 * Used to add a single Value to a Registered Expirable Cache with specific timeout
	 * @param k the key
	 * @param v the value
	 * @param timeOut the timeout for the key
	 * @param timeUnit the units in which timeout is calculated
	 * @param registeredExpirableCache The name with which the cache has been registered.
	 */
	 ...
	 * @param expirableCache The existing cache
	 * @param registeredExpirableCache The name with which the cache has been registered.
	 */
